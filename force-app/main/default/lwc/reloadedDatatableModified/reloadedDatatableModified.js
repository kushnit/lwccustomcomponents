...
export default class ReloadedDatatable extends LightningElement {
    ...
    connectedCallback(){
        this.loadRelatedRecords(this.accountId);
    ...
    handleValueChange(event) {        
        ...
        console.log('dataRecieved at line 209 of reloadedDatatable::',dataRecieved);
        let updatedItem;
        ...
        console.log('updatedItem at 228 of reloadedDatatable::',JSON.stringify(updatedItem));
        this.updateDraftValues(updatedItem);
        this.updateDataValues(updatedItem);
    }

    updateDataValues(updateItem) {
        console.log('this.records in updateDataValues before update',JSON.stringify(this.records));
        let copyData = JSON.parse(JSON.stringify(this.records));
        copyData.forEach((item) => {
            console.log('item.Id',item.Id);
            console.log('item.Amount',item.Amount);
            console.log('updateItem.Id',updateItem.Id);
            if (item.Amount === updateItem.Id) {
                for (let field in updateItem) {
                    console.log('field in updateItem', field);
                    item[field] = updateItem[field];
                }
            }
        });
        this.records = [...copyData]
        console.log('this.records in updateDataValues after update',JSON.stringify(this.records));
    }
    ...
    async handleSave(event) {
        ...
        console.log('updatedFields in handleSAve', updatedFields);
        this.draftValues = [];
